  ...
  <div class="audit-trail" :class="computedClasses">
    ...
    <div v-if="filterable" class="audit-trail__filters">
      ...
      <div class="audit-trail__filter-actions">
        <BankingButton variant="text" size="small" @click="clearFilters">
          Clear Filters
        </BankingButton>
      </div>
    </div>
    ...
    <div v-if="showSummary" class="audit-trail__summary">
      <div class="audit-trail__count">
        ...
        <span v-if="totalEntries !== filteredEntries.length"> ({{ totalEntries }} total) </span>
      </div>
      ...
      <div v-else>
        ...
        >
          ...
          <div class="audit-trail__entry-icon">
            <VAvatar :color="getEntryColor(entry)" :variant="getEntryVariant(entry)" size="32">
              <VIcon :icon="getEntryIcon(entry)" size="small" :color="getEntryIconColor(entry)" />
            </VAvatar>
          </div>
          ...
          <div class="audit-trail__entry-content">
                ...
                <VExpansionPanel>
                  ...
                  <VExpansionPanelText>
                    <div class="audit-trail__entry-detail-grid">
                      ...
                      >
                        <strong class="audit-trail__entry-detail-key"
                          >{{ formatDetailKey(key) }}:</strong
                        >
                        <span class="audit-trail__entry-detail-value">{{
            ...
            <div
              ...
              class="audit-trail__entry-changes"
            >
              <VExpansionPanels variant="accordion" flat>
                <VExpansionPanel>
                  <VExpansionPanelTitle>
                      ...
                      >
                        ...
                        <div class="audit-trail__change-values">
                          <div class="audit-trail__change-old">
                            ...
                            <code class="audit-trail__change-value">{{
                              formatDetailValue(change.oldValue)
                            }}</code>
                          </div>
                          <VIcon
                            icon="mdi-arrow-right"
                            size="small"
                            class="audit-trail__change-arrow"
                          />
            ...
            <div v-if="entry.riskLevel" class="audit-trail__entry-risk">
              <VChip :color="getRiskColor(entry.riskLevel)" size="small" variant="tonal">
                <VIcon :icon="getRiskIcon(entry.riskLevel)" size="small" start />
...
const filteredEntries = computed(() => {
  ...
  if (filters.value.dateFrom) {
    ...
    entries = entries.filter(entry => new Date(entry.timestamp) >= fromDate);
  }
  ...
  if (filters.value.riskLevel) {
    entries = entries.filter(entry => entry.riskLevel === filters.value.riskLevel);
  }

  if (filters.value.ipAddress) {
    entries = entries.filter(entry => entry.metadata?.ipAddress?.includes(filters.value.ipAddress));
  }
...
const totalPages = computed(() => Math.ceil(filteredEntries.value.length / props.itemsPerPage));
